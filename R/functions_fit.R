...
#' @param max.iter Number of maximum iterations used in the final run of the EM algorithm. Defaults to 1000.
...
PI3M.fit <- function(data, prog_model = 'prog ~ 1', prev_model='prev ~ 1', epsilon=1e-06, short.epsilon=1e-1, short.iter=10, short.runs=20, silent=T,  init=NULL, max.iter = 1000,
  ...
  CPU_time <- system.time({
    ...
    if (! inherits(data, "data.frame")) stop("Data is not supplied as a data frame. Please check and try again. ")
    ...
    clean_formula <- function(formula_str) { # Ensure space around "~" and "+"
    ...
    prog_model <- clean_formula(prog_model)
    prev_model <- clean_formula(prev_model )
    ...
    # require covariates to be corresponding to column names in the data
    ...
    em.function.h <- function(init, data, include.h, est.h=T, fixed.h=fixed.h, epsilon){
      ...
      convergence <- ifelse(iter > max.iter, F, T)
      return(list(model = list(prog_model, prev_model), initial.values = init, fixed.h = fixed.h, theta.hat = new_theta, num.iterations=iter,
                  log.likelihood = new_llk, hess=hess, grad = next_em_step[[3]], std.dev=std.dev, convergence=convergence, summary=summary))
    }
    ...
    if (is.null(init)){
      ...
      if (include.h & est.h)  pars <- c("h", pars)
    }
    ...
    if (any(is.na(final.res$std.dev))) warning("Hessian isn't positive definite, algorithm might not have converged to global maximum.")

  })


  final.res$time <- CPU_time
